...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
uint32_t getRenderBufferSize() { return state.renderBufSize; }

//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void unlockRenderBuffer() { pthread_mutex_unlock(&(state.mutex)); }
...
Java_com_example_cameranative_CameraSampleRenderer_render(JNIEnv* env,
  ...
  if (state.cameraRenderer && state.renderBufRGB565) {
    lockRenderBuffer();
    state.cameraRenderer->Render(state.renderBufRGB565, state.renderBufWidth,
