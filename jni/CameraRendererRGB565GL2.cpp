...
#define EPRINTF(...) \
  __android_log_print(ANDROID_LOG_ERROR, "CameraRendererRGB565GL2", __VA_ARGS__)
...
bool CameraRendererRGB565GL2::Render(const uint8_t* img, uint16_t w,
  ...
  // Unity does not unbind these buffers before we are called, so we have to do
  // so. This should shouldn't have an effect on other rendering solutions as
  // long as the programmer is binding these buffers properly
...
bool CameraRendererRGB565GL2::CreateShaderProgram() {
  ...
  if (!mProgramObject) return false;
...
bool CameraRendererRGB565GL2::CreateTextures() {
  ...
  for (size_t i = 0; i < 20; i++) mVertices[i] = mLandscapeVertices[i];
